...
export const createCourse = async (req, res) => {
    try {
        ...
        const formattedMaterials = Array.isArray(materials) ? materials.map((material) => ({
            ...material
        })) : [];
...
export const updateCourse = async (req, res) => {
    ...
    try {
        const material = Array.isArray(materials) ? materials.map((material) => ({
            ...material
        })) : [];
        ...
        if (!updateCourse) return res.status(404).json({
            message: 'Course not found'
        });
...
export const addThumbnailToCourse = async (req, res) => {
    try {
        ...
        const updatedCourse = await Course.findByIdAndUpdate(
            id,
            { thumbnail: thumbnailPath, updatedAt: Date.now() },
            { new: true }
        );
